"""
DeepLMI Cold-Start Training on Custom Dataset
===============================================
Modified from main_cold_start.py to work with dataset_custom.
Includes Doc2Vec caching and PyTorch model checkpointing.
"""
import os
import random
import string
...
parser = argparse.ArgumentParser(description="DeepLMI Cold Start")
parser.add_argument('--mode', type=str, default='both', choices=['standard', 'miRNA', 'lncRNA', 'both'], help='Testing mode to run')
args = parser.parse_args()

COLD_START = args.mode
print(f"Running DeepLMI in {COLD_START} mode...")

SAVE_PATH = "cold_data_splits_custom"
DOC2VEC_CACHE = "dataset_custom/doc2vec_cache.pkl"
CHECKPOINT_FILE = "checkpoint_deeplmi.pt"
    ...
    with open(file_path, 'r') as f:
        ...
        for line in f:
            ...
            if line.startswith(">"):
                ...
                curr_seq.append(line.translate(str.maketrans('', '', string.punctuation)))
        if curr_name is not None:
            names.append(curr_name)
            seqs.append("".join(curr_seq))
    return names, seqs

unique_lnc, lnc_seq = read_fasta(DATASET + '/lncrna.fasta')

# ----------- DOC2VEC CACHING -----------
if os.path.exists(DOC2VEC_CACHE):
    print("Loading Doc2Vec embeddings from cache... This saves ~1 hour!")
    with open(DOC2VEC_CACHE, 'rb') as f:
        vectors_3, vectors_5, vectors_7 = pickle.load(f)
else:
    print("Calculating Doc2Vec embeddings from scratch (This will take a while)...")
    lnc_seq_mers_3 = []
    for i in lnc_seq:
    ...
    print("Saving Doc2Vec embeddings to cache...")
    with open(DOC2VEC_CACHE, 'wb') as f:
        pickle.dump((vectors_3, vectors_5, vectors_7), f)
# ---------------------------------------
...
# get lncRNA and miRNA names
lncRNA_name_list, _ = read_fasta(DATASET + '/lncrna.fasta')
miRNA_name_list, _ = read_fasta(DATASET + '/mirna.fasta')
...
def test(data, miRNA_f, miRNA_name_list, lncRNA_f, lncRNA_name_list, lnc_index,mi_index):
    ...
    rna_to_index = pd.Series(graph_table['index'].values, index=graph_table['rna']).to_dict()
    X_t = torch.zeros(len(miRNA_name_list) + len(lncRNA_name_list), 100).to(device)
    ...
    return f1, roc_auc, avg_precision, ndcg, precision, recall, acc, emb, y_pred_new
...
if os.path.exists(CHECKPOINT_FILE):
    print("Found checkpoint file! Loading saved state...")
    checkpoint = torch.load(CHECKPOINT_FILE, weights_only=False)
    
    start_fold = checkpoint['fold']
    start_epoch = checkpoint['epoch'] + 1
    
    best_epoches = checkpoint['best_epoches']
    f1_scores = checkpoint['f1_scores']
    auc_scores = checkpoint['auc_scores']
    ap_scores = checkpoint['ap_scores']
    ndcg_scores = checkpoint['ndcg_scores']
    pre_scores = checkpoint['pre_scores']
    rec_scores = checkpoint['rec_scores']
    acc_scores = checkpoint.get('acc_scores', [])
    
    if start_epoch > EPOCH:
        start_fold += 1
        start_epoch = 1
        
    print(f"Resuming training from Fold {start_fold}, Epoch {start_epoch}")
else:
...
for fold in range(start_fold, 6):
    ...
    # Check if we need to load state inside this fold
    if os.path.exists(CHECKPOINT_FILE) and fold == start_fold:
        print(f"Restoring model state for Fold {fold}...")
        model.load_state_dict(checkpoint['model_state_dict'])
        miRNA_GCN.load_state_dict(checkpoint['miRNA_GCN_state_dict'])
        lncRNA_SA.load_state_dict(checkpoint['lncRNA_SA_state_dict'])
        model_k_f.load_state_dict(checkpoint['model_k_f_state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        results = checkpoint['results']
        best_val_auc = checkpoint['best_val_auc']
        current_epoch_start = start_epoch

    for epoch in range(current_epoch_start, EPOCH + 1):
        miRNA_GCN.train()
        model.train()
        lncRNA_SA.train()
        model_k_f.train()
        ...
        with torch.no_grad():
            ...
            val_f1, val_auc, val_ap, val_ndcg, val_pre, val_rec, val_acc, emb, label = test(val_data, miRNA_f_list_t,
        ...
        results.append([epoch, val_f1, val_auc, val_ap, val_ndcg, val_pre, val_rec, val_acc])
        ...
        torch.save({
            ...
            'miRNA_GCN_state_dict': miRNA_GCN.state_dict(),
            'lncRNA_SA_state_dict': lncRNA_SA.state_dict(),
            'model_k_f_state_dict': model_k_f.state_dict(),
    ...
    print(
    'Best result: Epoch: {}, F1: {:.3f}, AUC: {:.3f}, AP: {:.3f}, NDCG: {:.3f},Pre: {:.3f}, Rec: {:.3f}, Acc: {:.3f}'.format(
        *best_result))
    ...
    acc_scores.append(best_result[7])
...
print('Acc scores: mean {:.3f}, std {:.3f}'.format(
